...
# the enablessh.local script. The openEuler image already has the build's
...
# Under TCG emulation on a 2-core GHA runner a cold sshd start can take
# 30-60 s on its own; timeout 60 gives that slack. Iteration cap is 120
# (~30 min) for the case where the guest is genuinely cold-booting slowly.
_n=0
